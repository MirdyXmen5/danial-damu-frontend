        ...
        <div className="flex justify-between items-center h-16 md:h-20">
          ...
          <nav className="hidden md:flex space-x-8">
            {menuItems.map((item) => (
              <Link 
                key={item.key}
                to={item.path}
                className="text-brand-text-secondary hover:text-brand-brightBlue font-medium transition-colors"
              >
                {t(`header.${item.key}`)}
              </Link>
            ))}
          ...
          <div className="flex items-center space-x-4">
            <button 
              ...
              className="flex items-center gap-1 text-brand-text-secondary hover:text-brand-brightBlue transition-colors font-medium px-2 py-1 rounded-md"
            >
              <Globe size={20} />
            ...
            <div className="px-4 pt-2 pb-4 space-y-1 shadow-inner">
              {menuItems.map((item) => (
                <Link
                  key={item.key}
                  to={item.path}
                  className="block px-3 py-3 rounded-md text-base font-medium text-brand-text-secondary hover:text-brand-brightBlue hover:bg-brand-bg-softBlue transition-colors"
